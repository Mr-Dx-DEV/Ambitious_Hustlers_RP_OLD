...
-- This is the event if you want to add to radialmenu or other thing to check insurance
RegisterNetEvent("m-Insurance:Client:HaveCarInsurance", function ()
    local playerPed = PlayerPedId()
    local playerCoords = GetEntityCoords(playerPed)
    local vehicle = lib.getClosestVehicle(playerCoords, 5.0, true)

    if vehicle then
        local plate = GetVehicleNumberPlateText(vehicle)
        QBCore.Functions.TriggerCallback("m-Insurance:Server:HaveCarInsurance", function(hasInsurance)
            if hasInsurance then
                Notify(Language[Config.Language].hasInsurance, "success", 5000)
            else
                Notify(Language[Config.Language].hasntInsurance, "error", 5000)
            end
        end, plate)
    end
end)