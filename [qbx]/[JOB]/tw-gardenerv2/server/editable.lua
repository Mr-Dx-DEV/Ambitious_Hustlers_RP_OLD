...
discord_webhook = {
    ['jobfinish'] =
    "https://discord.com/api/webhooks/1431024805217566861/vPc137qWA5BNL1UXANQCSCLmtzWmpRRwBskDiPl5q_FyL7PsOPi3PrE4ipV7NdctOC5n",
}
        ...
        embeds = {
            {
                ...
                color = 0xFFA500,
                author = {
                    name = 'Tworst  Gardenerv2 - JOB FINISH',
                },
                thumbnail = {
                    url = data.avatar
                },
    ...
    if lobby.roomSetting.startJob then
        for _, player in ipairs(lobby.players) do
            if player.source == src then
                ...
                if lobby.roomSetting.mowerObject then
                    for k, v in pairs(lobby.roomSetting.mowerObject) do
                        if v.objectNetId then
                ...
                if lobby.roomSetting.prunegrassObject then
                    for k, v in pairs(lobby.roomSetting.prunegrassObject) do
                        if v.objectNetId then
                            local object = NetworkGetEntityFromNetworkId(v.objectNetId)
                            if DoesEntityExist(object) then
                                DeleteEntity(object)
                ...
                if lobby.roomSetting.flowerObject then
                    for k, v in pairs(lobby.roomSetting.flowerObject) do
                        if v.objectNetId then
                            local object = NetworkGetEntityFromNetworkId(v.objectNetId)
                            if DoesEntityExist(object) then
                                DeleteEntity(object)
                ...
                if lobby.roomSetting.branchObject then
                    for k, v in pairs(lobby.roomSetting.branchObject) do
                        if v.objectNetId then
                            local object = NetworkGetEntityFromNetworkId(v.objectNetId)
                            if DoesEntityExist(object) then
                                DeleteEntity(object)
                ...
                if lobby.roomSetting.ladderObject then
                    if lobby.roomSetting.ladderObject.ladderSettings then
                        for i, v in ipairs(lobby.roomSetting.ladderObject.ladderSettings) do
                            if v.ladderNetId then
                                local object = NetworkGetEntityFromNetworkId(v.ladderNetId)
                                if DoesEntityExist(object) then
                                    DeleteEntity(object)
                                end
                            end
                ...
                if Config.onlyOneGroup then
                    local regionID = lobby.roomSetting.Mission.regionID
                    if regionID then
                ...
                for _, remainingPlayer in ipairs(lobby.players) do
                    if remainingPlayer.source ~= src then
                        TriggerClientEvent('tworst-gardenerv2:client:resetjob', remainingPlayer.source)
                        TriggerClientEvent('tworst-gardenerv2:client:sendNotification', remainingPlayer.source,
